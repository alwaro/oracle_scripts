...
from 
	...
	from
		...
		left join (select owner,table_name,stale_stats,trunc(last_analyzed) lastStats from dba_tab_statistics) st on se.object_name = st.table_name
...
where 
	...
	and osuser like '%&&sos%'
	and sid like '%&&ssid%'
	and (sess.module like '%&&smod%' or sess.program like '%&&smod%')
